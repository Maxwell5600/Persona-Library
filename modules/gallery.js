...
export function createPersonaLibrary(container, adapter) {
     ...
     */
    function buildPreviewView(p, draft) {
        const wrap = el('div', { class: 'pl-preview-view' });

        const composed = adapter.composeDescription
        ...
        const infoBar = el('div', { class: 'pl-preview-info' }, [
            ...
            el('span', { text: `${composed.length.toLocaleString()} characters` }),
        ]);

        const hint = el('div', {
            class: 'pl-sections-hint',
            text: 'This is the exact text that gets folded into the prompt \u2014 core description plus every enabled section, exactly as SillyTavern will inject it. Macros like {{user}}/{{char}} are resolved later, at generation time, so they show up unresolved here.',
        });

        const output = el('pre', { class: 'pl-preview-output' }, [
            document.createTextNode(composed || '(Nothing yet \u2014 add a core description or an enabled section.)'),
        ]);

        wrap.append(infoBar, hint, output);
        return wrap;
    }
    ...
    function renderDetail() {
        ...
        const tabContent = detailTab === 'edit' && canEdit
            ? editFields
            : detailTab === 'preview'
                ? buildPreviewView(p, sectionsDraft)
