...
import { newItem, newGroup, normalizeVariants, composeVariants } from './variants.js';
...
export function createPersonaLibrary(container, adapter) {
    ...
    function buildBindingEditor(node, canEdit, markDirty) {
        ...
        function renderBody() {
            ...
            if (mode === 'character' || mode === 'chat') {
                ...
                const renderRefs = () => {
                    refsList.replaceChildren(...node.binding.refs.map((r, i) => el('span', { class: 'pl-binding-ref' }, [
                        el('span', { class: 'pl-binding-ref-label', text: r.label || '(unnamed)' }),
                        el('button', {
                            class: 'pl-binding-ref-remove', type: 'button', title: 'Unbind',
                            onclick: () => { markDirty(); node.binding.refs.splice(i, 1); renderRefs(); },
                        }, [el('i', { class: 'fa-solid fa-xmark' })]),
                    ])));
                };
                renderRefs();
                const addBtn = el('button', {
                    ...
                    text: mode === 'character' ? '+ Bind current character' : '+ Bind current chat',
                    onclick: async () => {
                        try {
                            if (mode === 'character') {
                                ...
                                if (!node.binding.refs.some((r) => r.uuid === key)) {
                                    markDirty();
                                    node.binding.refs.push({ uuid: key, label: info.name });
                                }
                            } else {
                                ...
                                if (!node.binding.refs.some((r) => r.uuid === uuid)) {
                                    markDirty();
                                    node.binding.refs.push({ uuid, label });
                                }
                            }
                        } catch (e) {
                ...
                if (!canEdit) addBtn.setAttribute('disabled', '');
                const hint = mode === 'character'
                    ? 'Active whenever any of these characters is the one currently loaded.'
                    : 'Active only in these exact chats \u2014 survives renaming or exporting/re-importing them.';
            ...
            if (mode === 'match') {
                const patternInput = el('input', {
                    ...
                    value: node.binding.pattern ?? '',
                    oninput: () => { markDirty(); node.binding.pattern = patternInput.value; },
                });
                if (!canEdit) patternInput.setAttribute('disabled', '');
    ...
    function buildPreviewView(p, draft, variants, adapter) {
        ...
        function contextFor(mode) {
            if (mode === 'character') {
                const info = adapter.getCurrentCharacterInfo?.();
                return { characterBindingId: info?.bindingId ?? (info ? `avatar:${info.avatar}` : null), chatBindingId: null, characterDescription: info?.description ?? '' };
            }
            if (mode === 'chat') {
                return { characterBindingId: null, chatBindingId: adapter.getChatBindingId?.() ?? null, characterDescription: '' };
            }
        ...
        const hint = el('div', {
            class: 'pl-sections-hint',
            text: 'This is the exact text that gets folded into the prompt \u2014 core description, enabled sections, and (only while generating, for real) any variants matching the picker above. Macros like {{user}}/{{char}} are resolved later, at generation time, so they show up unresolved here.',
        });

        wrap.append(infoBar, hint, modeSelect, output);
        return wrap;
    }
