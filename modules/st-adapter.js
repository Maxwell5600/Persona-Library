...
// Holds whatever we temporarily overwrote, so it can be put back exactly as
// it was even if the generation errors out or is stopped early. Only one
// in-flight swap is tracked at a time by design — see registerVariantGenerationHook.
...
function onGenerationStarted() {
    try {
        ...
        const composed = composeVariants(d.description ?? '', variants, activeVariantContext());
        if (composed === (d.description ?? '')) return; // nothing actually matched right now
        pendingRestore = { id, original: d.description ?? '' };
        d.description = composed;
        if (getActiveId() === id && typeof ctx()?.setPersonaDescription === 'function') {
            try { ctx().setPersonaDescription(); } catch { /* ignore */ }
        }
    } catch (e) {
        console.warn('[PersonaLibrary] variant injection failed; leaving the stored description untouched', e);
        pendingRestore = null;
    }
}
...
function onGenerationEnded() {
    ...
    const { id, original } = pendingRestore;
    pendingRestore = null;
    try {
        const pu = powerUser();
        const d = pu.persona_descriptions?.[id];
        if (d) d.description = original;
        if (getActiveId() === id && typeof ctx()?.setPersonaDescription === 'function') {
            try { ctx().setPersonaDescription(); } catch { /* ignore */ }
        }
    } catch (e) {
        console.warn('[PersonaLibrary] could not restore persona description after generation', e);
    }
}
...
export function registerVariantGenerationHook() {
    ...
    if (!es?.on) return () => {};
    const startEvents = [types.GENERATION_STARTED].filter(Boolean);
    const endEvents = [types.GENERATION_ENDED, types.GENERATION_STOPPED].filter(Boolean);
    for (const n of startEvents) es.on(n, onGenerationStarted);
    for (const n of endEvents) es.on(n, onGenerationEnded);
    return () => {
        for (const n of startEvents) es.removeListener?.(n, onGenerationStarted);
        for (const n of endEvents) es.removeListener?.(n, onGenerationEnded);
    };
}
